...
impl DataContext {
	pub async fn new(data_info: DatabaseInfo) -> Self {
		...
		if data_info.port == 0 {
			if data_info.instance.is_empty() {
				connection_string = format!("Server=tcp:{};Initial Catalog={};Persist Security Info=False;User ID={};Password={};MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;",
              &data_info.host,
              &data_info.database,
              &data_info.user_name,
              &data_info.password);
	...
	pub async fn new_from_connection(connection_string: String, using_named_connection: bool) -> Self {
		...
		let mut connection_manager = bb8_tiberius::ConnectionManager::new(config.clone());
		...
		DataContext { connection_pool: pool }
	}
}
